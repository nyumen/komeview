...
![komeview のスクリーンショット](docs/screenshot.png)
...
### macOS（Apple Silicon）
- `komeview.dmg` を開き、`komeview.app` を「アプリケーション」へドラッグ。
- 署名・公証をしていないため、初回起動時に Gatekeeper のブロックが出ます。
  - **「"komeview"は壊れているため開けません」と表示される場合**は、ターミナルで次を一度実行してください（検疫属性を外すだけで、アプリは壊れていません）：
    ```sh
    xattr -cr /Applications/komeview.app
    ```
    以降は通常どおり Dock から起動できます。
  - （環境によっては **アプリを右クリック →「開く」** で回避できることもあります）
...
右クリック →「コメントリスト」で、ウィンドウ右側にコメント一覧パネルを表示できます。

- 再生位置に追従してスクロール（パネルにマウスを乗せている間は停止）
- 行をクリックするとその位置へシーク
- 行を右クリックすると、コメント/ユーザーIDのコピー、**NG設定**（コメント本文・ユーザーID単位で非表示化）ができます
- NGにした行はリスト上で色分け表示され（ユーザー=オレンジ / コメント=紺）、同じメニューから解除できます
